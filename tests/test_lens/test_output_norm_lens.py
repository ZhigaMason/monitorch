...
@pytest.mark.parametrize(
    ...
    [
        ...
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'matplotlib', {'activation': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'tensorboard', {'activation': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'print', {'activation': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'matplotlib', {'include': [nn.Linear]}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'tensorboard', {'include': [nn.Linear]}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'print', {'include': [nn.Linear]}),
    ],
)
def test_output_norm_lens(module, loss_fn, visualizer, lens_kwargs):

    inspector = PyTorchInspector(lenses=[OutputNorm(**lens_kwargs)], module=module, visualizer=visualizer)
