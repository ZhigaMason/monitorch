...
@pytest.mark.parametrize(
    ...
    [
        ...
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Dropout(0.2), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'matplotlib', {'warning_plot': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Dropout(0.2), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'tensorboard', {'warning_plot': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Dropout(0.2), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'print', {'warning_plot': False}),
    ],
)
def test_parameter_gradient_activation(module, loss_fn, visualizer, lens_kwargs):

    inspector = PyTorchInspector(lenses=[ParameterGradientActivation(**lens_kwargs)], module=module, visualizer=visualizer)
