...
@pytest.mark.parametrize(
    ...
    [
        ...
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'matplotlib', {'comparison_plot': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'tensorboard', {'comparison_plot': False}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'print', {'comparison_plot': False}),
    ],
)
def test_parameter_norm_lens(module, loss_fn, visualizer, lens_kwargs):

    inspector = PyTorchInspector(lenses=[ParameterNorm(**lens_kwargs)], module=module, visualizer=visualizer)
