...
@pytest.mark.parametrize(
    ...
    [
        ...
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'tensorboard', {}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'print', {}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'matplotlib', {'separate_loss_and_metrics': True}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'tensorboard', {'separate_loss_and_metrics': True}),
        (nn.Sequential(nn.Linear(N_DIM, 32), nn.ReLU(), nn.Linear(32, 1), nn.Sigmoid()), nn.BCELoss(), 'print', {'separate_loss_and_metrics': True}),
    ],
)
